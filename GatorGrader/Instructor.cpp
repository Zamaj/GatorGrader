	...
	//if user is an instructor
		...
		while (getline(file, foundCourseData)) {
			...
			if (foundCourseData.at(foundCourseName.size()) == '$') {
				...
				currentStudent = currentCourse->addStudent(foundFirstName, foundLastName, foundCourseData);
				continue;
			}
			...
			if (foundCourseData.at(foundCourseName.size()) == '@') {
				...
				{
					if (currentCourse->getStudentList()[i]->getFullName() == currentStudent->getFullName()) 
					{
